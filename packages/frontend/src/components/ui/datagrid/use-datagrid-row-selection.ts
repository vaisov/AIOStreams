...
export type DataGridOnRowSelect<T> = (
  event: DataGridRowSelectedEvent<T>
) => void;
...
export function useDataGridRowSelection<T extends Record<string, any>>(
  props: DataGridRowSelectionProps<T>
) {
  const {
    table,
    data,
  ...
  React.useEffect(() => {
    if (
      ...
      !displayedRows.some((row) => !!row.original[key])
    ) {
      console.error(
        '[DataGrid] The key provided by `rowSelectionPrimaryKey` does not match any property in the data.'
      );
    ...
    if (data && data?.length > 0 && canSelect.current && !!key) {
      ...
      if (persistent) {
        ...
        onRowSelect &&
          onRowSelect({
            data:
              Array.from(selectedRowsRef.current.values()).filter((v: any) =>
                selectedKeys.has(v[key])
              ) ?? [],
          });
      } else {
        onRowSelect &&
