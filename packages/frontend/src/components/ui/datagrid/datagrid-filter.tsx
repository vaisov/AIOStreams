...
export const DataGridFilterAnatomy = defineStyleAnatomy({
  root: cva(['UI-DataGridFilter__root', 'flex items-center max-w-full gap-2']),
});
...
export type DataGridFilterProps<T extends Record<string, any>> =
...
export function DataGridFilter<T extends Record<string, any>>(
  props: DataGridFilterProps<T>
) {
  const { children, className, column, onRemove, lng = 'en', ...rest } = props;
  ...
  const setFilterValue = React.useMemo(
  ...
  const valueFormatter =
    filterParams.valueFormatter || getValueFormatter(column);
  ...
  return (
    <div className={cn(DataGridFilterAnatomy.root(), className)} {...rest}>
      {filterParams.type === 'select' && (!options || options.length === 0) && (
        <div className="text-red-500">
          /!\ "Select" filtering option passed without options
        </div>
      )}
      {/*Select*/}
      {filterParams.type === 'select' && !!options && options.length > 0 && (
        ...
        >
          <DropdownMenuGroup>
            <DropdownMenuItem onClick={() => handleUpdate(true)}>
              {typeof valueFormatter(true) === 'boolean'
                ? translations['true'][lng]
                : valueFormatter(true)}
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleUpdate(false)}>
              {typeof valueFormatter(false) === 'boolean'
                ? translations['false'][lng]
      ...
      <CloseButton intent="gray-outline" onClick={onRemove} size="md" />
    </div>
  );
}
...
interface DataGridActiveFilterProps
  extends
    Omit<React.ComponentPropsWithRef<'button'>, 'value'>,
...
export const DataGridActiveFilter = React.forwardRef<
  HTMLButtonElement,
  DataGridActiveFilterProps
>((props, ref) => {
  ...
  return (
    ...
    >
      ...
      <span className="font-semibold flex flex-none overflow-hidden whitespace-normal">
        {displayedValue}
      </span>
    </button>
  );
