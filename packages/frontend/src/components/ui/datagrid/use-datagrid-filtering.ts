...
export function useDataGridFiltering<T>(props: DataGridFilteringHookProps<T>) {
  const { table, columnFilters } = props;
  ...
  const [filterableColumns, filteredColumns] = React.useMemo(() => {
    return [
      table
        ...
        .filter(
          (col) =>
            col.getCanFilter() && !!getColumnHelperMeta(col, 'filteringMeta')
        ),
      table
        .getAllLeafColumns()
        .filter((col) =>
          columnFilters.map((filter) => filter.id).includes(col.id)
        ),
    ];
  }, [table.getAllLeafColumns(), columnFilters]);
  const unselectedFilterableColumns = filterableColumns.filter(
    (n) => !columnFilters.map((c) => c.id).includes(n.id)
  );
...
export const dateRangeFilter = (
  ...
  return (
    (value >= filterValue.start && value <= filterValue.end) ||
    isSameDay(value, filterValue.start) ||
    isSameDay(value, filterValue.end)
  );
};
