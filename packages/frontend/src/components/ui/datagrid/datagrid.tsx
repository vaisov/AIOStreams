...
import {
  DataGridApi,
  DataGridInstanceProps,
  useDataGrid,
} from './datagrid-instance';
...
export const DataGridAnatomy = defineStyleAnatomy({
  ...
  table: cva(['UI-DataGrid__table', 'w-full relative table-fixed']),
  tableHead: cva(['UI-DataGrid__tableHead', '']),
  th: cva([
  ...
  tableBody: cva(['UI-DataGrid__tableBody', 'w-full relative border-b']),
  td: cva([
    'UI-DataGrid__td',
  ...
  tr: cva(['UI-DataGrid__tr', 'hover:bg-[--subtle] truncate']),
  footer: cva([
    'UI-DataGrid__footer',
...
export type DataGridProps<T extends Record<string, any>> = ComponentAnatomy<
  typeof DataGridAnatomy
> &
  DataGridInstanceProps<T> & {
...
export function DataGrid<T extends Record<string, any>>(
  ...
  const isInLoadingState =
    isLoading || (!enableOptimisticUpdates && isDataMutating);
  const { tableRef } = useDataGridResponsiveness({ table, hideColumns });

  const { selectedRowCount } = useDataGridRowSelection({
    table: table,
    data: data,
  ...
  return (
    <div className={cn(DataGridAnatomy.root(), className)}>
        ...
        <div className={cn(DataGridAnatomy.toolbar(), toolbarClass)}>
          ...
          {unselectedFilterableColumns.length > 0 && (
            <DropdownMenu
              trigger={
                <button
                  className={cn(
                    DataGridAnatomy.filterDropdownButton(),
                    filterDropdownButtonClass
                  )}
                >
                  <svg
                  ...
                  <span>
                    {translations['filters'][lng]} (
                    {unselectedFilterableColumns.length})
                  </span>
                </button>
              }
            >
          ...
          {selectedRowCount > 0 && (
            <div className="text-sm">
              {selectedRowCount}{' '}
              {
                translations[`row${selectedRowCount > 1 ? 's' : ''}-selected`][
                  lng
                ]
              }
            </div>
            ...
            {filteredColumns.map((col) => {
              return (
                <DataGridFilter
                  ...
                  column={col}
                  onRemove={() =>
                    handleColumnFiltersChange((filters) => [
                      ...filters.filter((filter) => filter.id !== col.id),
                    ])
                  }
                  lng={lng}
                />
      ...
      <div
        ref={tableRef}
        className={cn(DataGridAnatomy.tableContainer(), tableContainerClass)}
      >
                      ...
                      >
                        {header.isPlaceholder ? null : (
                          ...
                          >
                            ...
                            <span
                              className={cn(
                                DataGridAnatomy.titleChevronContainer(),
                                titleChevronContainerClass
                              )}
                            >
                ...
                <tr key={row.id} className={cn(DataGridAnatomy.tr(), trClass)}>
                  {row.getVisibleCells().map((cell, index) => {
                    ...
                    const isCurrentlyEditable =
                      getIsCellEditable(cell.id) &&
                      !getIsCellActivelyEditing(cell.id) &&
                      (!getIsCurrentlyEditing() ||
                        getFirstCellBeingEdited()?.rowId === cell.row.id);
                    ...
                    return (
                      <td
                        ...
                        data-row-selected={cell
                          .getContext()
                          .row.getIsSelected()} // If cell's row is currently selected
                      ...
                      >
                        ...
                        {getIsCellActivelyEditing(cell.id) && (
                          <DataGridCellInputField
                            ...
                            rowErrors={rowErrors}
                            meta={
                              getColumnHelperMeta(cell.column, 'editingMeta')!
                            }
      ...
      <div className={cn(DataGridAnatomy.footer(), footerClass)}>
        ...
        <div
          className={cn(
            DataGridAnatomy.footerPageDisplayContainer(),
            footerPageDisplayContainerClass
          )}
        >
        ...
        <div
          className={cn(
            DataGridAnatomy.footerPaginationInputContainer(),
            footerPaginationInputContainerClass
          )}
        >
          {data.length > 0 && (
          ...
          <Select
            ...
            options={[
              ...
              ...[5, 10, 20, 30, 40, 50].filter(
                (n) => n !== Number(table.getState().pagination.pageSize)
              ),
            ].map((pageSize) => ({
...
export type DataGridWithApiProps<T extends Record<string, any>> =
  ComponentAnatomy<typeof DataGridAnatomy> & {
    api: DataGridApi<T>;
  };

export function DataGridWithApi<T extends Record<string, any>>(
  props: DataGridWithApiProps<T>
) {
  const { api, ...rest } = props;

  const { data, rowCount, columns } = api;
