...
export const DataGridCellInputFieldAnatomy = defineStyleAnatomy({
  root: cva(['UI-DataGridCellInputField__root']),
});
 ...
 * @internal
 */
export type DataGridCellInputFieldProps<T extends Record<string, any>> =
  ComponentAnatomy<typeof DataGridCellInputFieldAnatomy> & {
    /**
...
>(props: DataGridCellInputFieldProps<T>) {
  const {
    ...
    rowErrors,
    onValueUpdated,
    meta: { field, zodType, valueFormatter: _valueFormatter },
  } = props;
  const defaultValueFormatter = (value: any) => value;
  const valueFormatter = (_valueFormatter ?? defaultValueFormatter) as (
    value: any
  ) => any;
