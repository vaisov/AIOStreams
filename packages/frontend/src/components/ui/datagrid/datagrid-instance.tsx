...
import {
  DataGridOnRowEdit,
  DataGridOnRowValidationError,
} from './use-datagrid-editing';
...
export function useDataGrid<T extends Record<string, any>>(
  ...
  const [globalFilter, setGlobalFilter] = React.useState<string>(
    initialState?.globalFilter ?? defaultValues.globalFilter
  );
  const [rowSelection, setRowSelection] = React.useState<RowSelectionState>(
    initialState?.rowSelection ?? defaultValues.rowSelection
  );
  const [sorting, setSorting] = React.useState<SortingState>(
    initialState?.sorting ?? defaultValues.sorting
  );
  const [columnFilters, setColumnFilters] = React.useState<ColumnFiltersState>(
    initialState?.columnFilters ?? defaultValues.columnFilters
  );
  const [columnVisibility, setColumnVisibility] =
    React.useState<VisibilityState>(
      initialState?.columnVisibility ?? defaultValues.columnVisibility
    );
  const [pagination, setPagination] = React.useState<PaginationState>(
    initialState?.pagination ?? defaultValues.pagination
  );
  ...
  const globalFilterState = React.useMemo(
  ...
  const table = useReactTable<T>({
    ...
    onColumnFiltersChange: changeHandler(
      onColumnFiltersChange,
      setColumnFilters
    ),
    onColumnVisibilityChange: changeHandler(
      onColumnVisibilityChange,
      setColumnVisibility
    ),
    getCoreRowModel: getCoreRowModel(),
    getSortedRowModel: enableManualSorting ? undefined : getSortedRowModel(),
    getFilteredRowModel: enableManualFiltering
      ? undefined
      : getFilteredRowModel(),
    filterFns: {
    ...
    getRowId: !!props.rowSelectionPrimaryKey
      ? (row) => row[props.rowSelectionPrimaryKey!]
      : undefined,
  });
  ...
  const displayedRows = React.useMemo(() => {
    ...
    return table
      .getRowModel()
      .rows.slice(pn.pageIndex * pn.pageSize, (pn.pageIndex + 1) * pn.pageSize);
...
export type DataGridApi<T extends Record<string, any>> = ReturnType<
  typeof useDataGrid<T>
>;
