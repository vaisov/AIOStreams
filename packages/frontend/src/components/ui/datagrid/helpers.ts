...
import { AnyZodObject, z, ZodAny, ZodTypeAny } from 'zod/v3';
...
export type DataGridEditingHelper<
  T extends any = unknown,
  ZodType extends ZodTypeAny = ZodAny,
> = {
  zodType?: ZodType;
  field: (
    context: DataGridEditingFieldContext<
      ZodType extends ZodAny ? T : z.infer<ZodType>
    >,
    options: {
...
function withEditing<
  T extends any = unknown,
  ZodType extends ZodTypeAny = ZodAny,
>(params: DataGridEditingHelper<T, ZodType>) {
  return {
    editingMeta: {
      ...params,
    },
...
export type DataGridFilteringType =
  | 'select'
  | 'radio'
  | 'checkbox'
  | 'boolean'
...
type DefaultFilteringProps<T extends DataGridFilteringType> = {
  ...
  options: {
    value: string;
    label?: T extends 'select' ? string : React.ReactNode;
  }[];
  valueFormatter?: (value: any) => any;
};

// Improve type safety by removing "options" when the type doesn't need it
export type DataGridFilteringHelper<
  T extends DataGridFilteringType = 'select',
> =
...
export type DataGridSupportedFilterFn =
  | Extract<
      BuiltInFilterFn,
      'equals' | 'equalsString' | 'arrIncludesSome' | 'inNumberRange'
    >
  | 'dateRangeFilter';

function withFiltering<T extends DataGridFilteringType>(
  params: DataGridFilteringHelper<T>
) {
  return {
    filteringMeta: {
      ...params,
...
function withValueFormatter<T extends any, R extends any = any>(
  callback: (value: T) => R
) {
  return {
...
export function getValueFormatter<T>(column: Column<T>): (value: any) => any {
  return (
    (column.columnDef.meta as any)?.valueFormatter || ((value: any) => value)
  );
}
 ...
 * @param callback
 */
export function defineDataGridColumns<
  T extends Record<string, any>,
  Schema extends AnyZodObject = any,
>(
  callback: (
    helpers: DataGridColumnDefHelpers<T>,
    schema?: Schema
  ) => Array<ColumnDef<T>>
) {
  return callback({
...
export function getColumnHelperMeta<T, K extends DataGridHelpers>(
  column: Column<T>,
  helper: K
) {
  return (column.columnDef.meta as any)?.[helper] as
    | (K extends 'filteringMeta'
        ? _DefaultFilteringProps
        : K extends 'editingMeta'
          ? DataGridEditingHelper
          : K extends 'valueFormatter'
            ? ReturnType<typeof withValueFormatter>
            : never)
    | undefined;
}
