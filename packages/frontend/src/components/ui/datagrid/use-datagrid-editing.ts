...
import { AnyZodObject, ZodIssue } from 'zod/v3';
 ...
 */
export type DataGridOnRowEdit<T extends Record<string, any>> = (
  event: DataGridRowEditedEvent<T>
) => void;
 ...
 */
export type DataGridOnRowValidationError<T extends Record<string, any>> = (
  event: DataGridRowValidationError<T>
) => void;

//----

export type DataGridValidationRowErrors = Array<{
  rowId: string;
  key: string;
  message: string;
}>;
...
export function useDataGridEditing<T extends Record<string, any>>(
  props: Props<T>
) {
  const {
    data,
  ...
  const editableColumns = React.useMemo(() => {
    return leafColumns.filter(
      (n) => n.getIsVisible() && !!(n.columnDef.meta as any)?.editingMeta
    );
  }, [leafColumns]);
    ...
    if (rows.length > 0) {
      return rows.flatMap((row) =>
        row
          ...
          .filter(
            (cell) =>
              !!editableColumns.find((col) => col.id === cell.column.id)?.id
          )
      );
    }
    return [];
  ...
  const handleStartEditing = React.useCallback((cellId: string) => {
    ...
    setEditableCellStates((prev) => {
      ...
      if (cell && prev.every((prevCell) => !prevCell.isEditing)) {
        // (Event 1) When we select a cell and nothing else is being edited
        return [...others, { ...cell, id: cellId, isEditing: true }];
      } else if (cell && prev.some((prevCell) => prevCell.isEditing)) {
        // (Event 2) When another cell is being edited
        const otherCellBeingEdited = prev.find(
          (prevCell) => prevCell.isEditing
        ); // Find the cell being edited

        if (otherCellBeingEdited?.rowId === cell?.rowId) {
          // Only allow cells on the same row to be edited
          return [...others, { ...cell, id: cellId, isEditing: true }];
        }
      }
      return prev;
  ...
  /**/
  const getIsCellActivelyEditing = React.useCallback(
    (cellId: string) => {
      return editableCellStates.some(
        (cell) => cell.id === cellId && cell.isEditing
      );
    },
    [editableCellStates]
  );
  /**/
  const getIsCellEditable = React.useCallback(
    (cellId: string) => {
      ...
      if (!equal(rowData, row.original)) {
        ...
        if (enableOptimisticUpdates && optimisticUpdatePrimaryKey) {
          ...
          const index = clone.findIndex((p) => {
            if (
              !p[optimisticUpdatePrimaryKey] ||
              !rowData[optimisticUpdatePrimaryKey]
            )
              return false;
            return (
              p[optimisticUpdatePrimaryKey] ===
          ...
          if (clone[index] && index > -1) {
            ...
            console.error(
              '[DataGrid] Could not perform optimistic update. Make sure `optimisticUpdatePrimaryKey` is a valid property.'
            );
          }
        } else if (enableOptimisticUpdates) {
          console.error(
            '[DataGrid] Could not perform optimistic update. Make sure `optimisticUpdatePrimaryKey` is defined.'
          );
        }
   ...
   */
  const handleUpdateValue = React.useCallback<DataGridEditingValueUpdater<T>>(
    (value, _row, cell, zodType) => {
