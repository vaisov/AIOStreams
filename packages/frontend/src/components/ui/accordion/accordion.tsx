...
export const AccordionAnatomy = defineStyleAnatomy({
  root: cva(['UI-Accordion__root']),
  header: cva(['UI-Accordion__header', 'flex text-lg']),
  trigger: cva([
  ...
  content: cva(['UI-Accordion__content', 'p-4']),
});
...
const __AccordionAnatomyContext = React.createContext<
  ComponentAnatomy<typeof AccordionAnatomy>
>({});
...
>((props, ref) => {
  const { className, headerClass, triggerIconClass, children, ...rest } = props;
  ...
  return (
    <AccordionPrimitive.Header
      className={cn(AccordionAnatomy.header(), _headerClass, headerClass)}
    >
      <AccordionPrimitive.Trigger
        ref={ref}
        className={cn(AccordionAnatomy.trigger(), _triggerClass, className)}
        {...rest}
      >
        {children}
        <svg
          className={cn(
            AccordionAnatomy.triggerIcon(),
            _triggerIconClass,
            triggerIconClass
          )}
...
>((props, ref) => {
  ...
  return (
    <AccordionPrimitive.Content
      ref={ref}
      className={cn(
        AccordionAnatomy.contentContainer(),
