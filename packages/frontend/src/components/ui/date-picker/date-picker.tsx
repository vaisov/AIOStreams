...
export const DatePickerAnatomy = defineStyleAnatomy({
  root: cva(['UI-DatePicker__root', 'line-clamp-1 text-left']),
  placeholder: cva(['UI-DatePicker__placeholder', 'text-[--muted]']),
});
...
export const DatePicker = React.forwardRef<HTMLButtonElement, DatePickerProps>(
  (props, ref) => {
    const [props1, basicFieldProps] = extractBasicFieldProps<DatePickerProps>(
      props,
      React.useId()
    );
    ...
    const Input = (
      ...
      >
        {date ? (
          ...
          <span
            className={cn(DatePickerAnatomy.placeholder(), placeholderClass)}
          >
    ...
    const Picker = (
      <div>
        {!hideYearSelector && (
          <div className="flex items-center justify-between p-1 sm:border-b">
            <Select
              size="sm"
              intent="filled"
              options={Array(getYear(new Date()) - 1699)
                .fill(0)
                .map((_, i) => ({
                  label: String(getYear(new Date()) + 100 - i),
                  value: String(getYear(new Date()) + 100 - i),
                }))}
              value={String(getYear(date ?? new Date()))}
              onValueChange={(value) =>
                setDate(setYear(date ?? new Date(), Number(value)))
              }
            />
          </div>
        )}
      ...
      <BasicField {...basicFieldProps}>
        <InputContainer {...inputContainerProps}>
          ...
          <div className="hidden sm:block w-full">
            <Popover className="w-auto p-0" trigger={Input}>
              {Picker}
            </Popover>
          </div>

          <div className="block sm:hidden w-full">
            <Modal title={placeholder || 'Select a date'} trigger={Input}>
              {Picker}
            </Modal>
          </div>
