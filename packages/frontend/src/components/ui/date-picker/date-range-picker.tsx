...
export const DateRangePickerAnatomy = defineStyleAnatomy({
  root: cva(['UI-DateRangePicker__root', 'line-clamp-1 text-left']),
  placeholder: cva(['UI-DateRangePicker__placeholder', 'text-[--muted]']),
});
...
>((props, ref) => {
  const [props1, basicFieldProps] =
    extractBasicFieldProps<DateRangePickerProps>(props, React.useId());
  ...
  const Input = (
    ...
    >
      {date?.from ? (
        date.to ? (
          <span className="line-clamp-1">{`${format(date.from, 'P')} - ${format(date.to, 'P')}`}</span>
        ) : (
          format(date.from, 'PPP')
        )
      ) : (
        <span
          className={cn(DateRangePickerAnatomy.placeholder(), placeholderClass)}
        >
    ...
    <BasicField {...basicFieldProps}>
      <InputContainer {...inputContainerProps}>
        ...
        <input
          ...
          value={
            date
              ? `${date.from?.toISOString()?.split('T')?.[0]}${date.to ? ',' + date.to.toISOString().split('T')[0] : ''}`
              : ''
          }
          aria-hidden="true"
