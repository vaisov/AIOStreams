...
export const AutocompleteAnatomy = defineStyleAnatomy({
  root: cva(['UI-Autocomplete__root']),
  popover: cva([
  ...
  container: cva(['UI-Autocomplete__container', 'relative w-full']),
  command: cva([
...
> &
  ...
  ComponentAnatomy<typeof AutocompleteAnatomy> & {
     ...
     */
    onValueChange?: (
      value: { value: string | null; label: string } | undefined
    ) => void;
    /**
...
export const Autocomplete = React.forwardRef<
  ...
  AutocompleteProps
>((props, ref) => {
  const [props1, basicFieldProps] = extractBasicFieldProps<AutocompleteProps>(
    props,
    React.useId()
  );
  ...
  const inputValueRef = React.useRef<string>(
    controlledValue?.label || defaultValue?.label || ''
  );
  ...
  const optionsTypeValueRef = React.useRef<AutocompleteOption | undefined>(
  ...
  const filteredOptions = React.useMemo(() => {
    if (autoFilter) {
      return options.filter((option) =>
        option.label.toLowerCase().includes(deferredInputValue.toLowerCase())
      );
    }
    return options;
  ...
  const _optionListShouldOpen =
    !!emptyMessage || (options.length > 0 && filteredOptions.length > 0);

  // Function used to compare two labels
  const by = React.useCallback(
  ...
  const handleOnTextInputChange = React.useCallback(
  ...
  const _updateOptionsTypeValueRef = React.useCallback(
    (value: AutocompleteOption | undefined) => {
    ...
    <BasicField {...basicFieldProps}>
      <InputContainer {...inputContainerProps}>
        ...
        <Popover
          ...
          className={cn(AutocompleteAnatomy.popover(), popoverClass)}
          onOpenAutoFocus={(e) => e.preventDefault()}
          trigger={
            <div
              className={cn(AutocompleteAnatomy.container(), containerClass)}
            >
              <input
        ...
        >
          ...
          >
            {isFetching && inputValue.length > 0 && (
              <div className="w-full absolute top-0 left-0 px-1">
                ...
                {options.map((option) => (
                  <CommandItem
                    ...
                    onSelect={(currentValue) => {
                      const _option = options.find((n) =>
                        by(n.label, currentValue)
                      );
                      if (_option) {
